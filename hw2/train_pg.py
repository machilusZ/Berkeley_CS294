...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        ...
        for layer in range(1, n_layers + 1):
            cur_output_size = output_size if layer == n_layers else size
            cur_activation = output_activation if layer == n_layers else activation
            prev_layer = tf.layers.dense(
                prev_layer, cur_output_size, activation=cur_activation, name='hidden_layer%d'%layer
            )
        return prev_layer
...
def train_PG(exp_name='',
    ...
    loss = tf.reduce_mean(sy_adv_n * -sy_logprob_n)
    ...
    for itr in range(n_iter):
        ...
        for path in paths:
            ...
            if reward_to_go:
                q = [
                    np.sum(np.power(gamma, np.arange(max_step - t)) * r[t:])
                    for t in range(max_step)
                ]
            else:
                q = [
                        np.sum(np.power(gamma, np.arange(max_step)) * r)
                    ] * max_step
        q_n += q
        ...
        if normalize_advantages:
            ...
            # YOUR_CODE_HERE
            adv_n_mean = np.mean(adv_n, axis=0)
            adv_n_std = np.std(adv_n, axis=0)
            adv_n = (adv_n - adv_n_mean) / (adv_n_std + 1e-7)
